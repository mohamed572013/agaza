    ...
    var Programs = function () {
        ...
        var handleChangeMainCategories = function () {
            $("#parent_category_id").change(function () {
                ...
                $.ajax({
                    type: "post",
                    url: config.admin_url + '/programs_agaza/getSubCategories',
        ...
        var handleChangeCountriesOrCities = function () {
            $("#country_id").change(function () {
                ...
                $.ajax({
                    type: "post",
                    url: config.admin_url + '/program_agaza/getCountryCities',
        ...
        var getPlaces = function (place_id, selected_id, content) {
            $.ajax({
                type: "post",
                url: config.admin_url + '/program_agaza/getPlaces',
                data: {
                    place_id: place_id,
        ...
        var getSubCategories = function (parent_category_id, selected_id, content) {
            $.ajax({
                type: "post",
                url: config.admin_url + '/programs_agaza/getSubCategoriesForEdit',
                data: {
        ...
        var handleRecords = function () {
            Programs_grid = $('#programs_table .dataTable').dataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": config.admin_url + "/program_agaza/data",
                    "type": "POST"
                },
                "columns": [
...
//                    },
            ...
            $('#addEditProgramsForm').submit(function () {
                var id = $('#id').val();
                var action = config.admin_url + '/program_agaza/add';
                if (id != 0) {
                    action = config.admin_url + '/program_agaza/edit';
                }
        ...
        var handleImagesFormSubmit = function () {
            ...
            var action = config.admin_url + '/program_agaza/add_images';
        ...
        var handleListImagesOnServer = function (program_id) {
            var action = config.admin_url + '/program_agaza/listFiles';
            $.ajax({
                url: action,
                ...
                success: function (data) {
                    ...
                    {
                        var items = [];
                        for (var x = 0; x < data.data.find.length; x++) {

                            items.push('<div style="position:relative;float:right;padding: 5px 5px;"><img style="height:80px;width:80px;" src="' + data.data.agazabook_fields.slider_url + '/uploads/programs_slider/' + data.data.find[x].image + '"/></div>');
                        }
        ...
        var remove_image = function () {
            $(document).on('click', '.program_image', function () {
                ...
                var action = config.admin_url + '/program_agaza/remove_image';
                $.ajax({
                    url: action,
                    data: {
                        image_id: image_id,
        ...
        return {
            ...
            edit_programs: function (t) {
                ...
                App.editForm({
                    element: t,
                    url: config.admin_url + '/program_agaza/row',
                    ...
                    success: function (data)
                    {
                        ...
                        {
                            if (i == 'image') {
                                $('.image_uploaded').html('<img style="height:80px;width:150px;" id="image_upload_preview" src="' + data.message.image_url + 'uploads/programs/' + data.message['agaza_image'] + '" alt="your image" />');
                               ...
                               // getSubCategories(data.message[i], data.message.category_id, 'category_id');
                                ...
                                $('#' + i).val(data.message[i]);
                                $('#slider-image-upload-box').slideDown(500);
                                $('.slider_image_uploaded').html('<img style="height:80px;width:150px;" id="image_upload_preview" src="' + config.base_url + 'uploads/programs_slider/' + data.message['slider_image'] + '" alt="your image" />');
            ...
            delete_programs: function (t) {
                    ...
                    buttons: {
                        confirm: {
                            ...
                            action: function () {
                                App.deleteForm({
                                    element: t,
                                    url: config.admin_url + '/program_agaza/delete',
                                    data: {id: $(t).attr("data-id")},
                                    success: function (data)
                                    {
                                        $.alert(data.message);
             ...
             activate_programs: function (t) {
               
                $.ajax({
                    type: "POST",
                    url: config.admin_url + '/program_agaza/activate',
                    data: {id: $(t).attr("data-id"), status: $(t).data("status")},
                    success: function (data) {
                        if(data.type=="success") {
                            Programs_grid.api().ajax.reload();
            ...
            programs_empty: function () {
                ...
                $('#places_id').find('option').eq(0).prop('selected', true);

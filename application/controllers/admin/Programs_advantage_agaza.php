...
class Programs_advantage extends C_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('Programs_advantage_model', 'Programs_advantage');
    }

    public function index() {
        $this->data['features_image'] = $this->Programs_advantage->GetWhere("features_image", "id", "ASC", array());
        $main_content = 'program_advantages/index';
    ...
    public function row() {
        // pri($_POST);
        $id = $_POST['id'];
        $find = $this->Programs_advantage->find($id);
    ...
    public function add() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            //pri($data_array);
            $add = $this->Programs_advantage->add($data_array);
            if ($add) {
    ...
    public function edit() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $where_array['id'] = $id;
            //pri($data_array);
            $update = $this->Programs_advantage->update($data_array, $where_array);
            if ($update) {
    ...
    public function delete() {
        $id = $_POST['id'];
        $where_array['id'] = $id;
        $delete = $this->Programs_advantage->delete($where_array);
        if ($delete != "done") {
            if ($delete == "programs_advantage_all") {
                print_json('success', _lang('not_valid_there_are_programs_advantage_all'));
            } else if ($delete == "data") {
    ...
    function data() {
        ...
        $this->datatables->add_column('options', function($data) {
            ...
            $back .= '<a href="#" title="' . _lang("edit") . '" class="tooltips" onclick="Programs_advantages.edit(this);return false;" data-id="' . $data["id"] . '"><i class="fa fa-2x fa-edit"></i></a>';
            $back .= "&nbsp;&nbsp;";
            ...
            $back .= '<a href="#"
                ...
                class="tooltips"
                data-id="' . $data["id"] . '"
                onclick="Programs_advantages.delete(this);return false;"><i class="fa fa-times fa-2x text-danger"></i></a>';
    ...
    public function titles_check($titles = array(), $id = false) {
        ...
        foreach ($titles as $key => $title) {
            ...
            $find = $this->Programs_advantage->findTitle($where_array);
            //pri($find);
            if ($find) {

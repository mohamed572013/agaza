<?php
    ...
    class Visa_types extends C_Controller{
        public function __construct(){
            parent::__construct();
            $this->load->model('Visa_types_model', 'visa_types');
        }

        public function index(){
            $main_content = 'visa_types/index';
        ...
        public function add(){
            ...
            if ($this->form_validation->run() == false) {
                ...
                //pri($data_array);
                $add = $this->visa_types->add($data_array);
                if ($add) {
        ...
        public function edit(){
            ...
            if ($this->form_validation->run() == false) {
                ...
                $where_array['id'] = $id;
                //pri($data_array);
                $update = $this->visa_types->update($data_array, $where_array);
                if ($update) {
        ...
        public function delete(){
            $id = $_POST['id'];
            $where_array['id'] = $id;
            $deleted = $this->visa_types->delete($where_array);
            if ($deleted) {
        ...
        function data(){
            ...
            $this->datatables->add_column('options', function($data) {
                ...
                $back .= '<a href="#" title="' . _lang("edit_room_classes") . '" class="tooltips" onclick="Visa_types.edit(this);return false;" data-id="' . $data["id"] . '"><i class="fa fa-2x fa-edit"></i></a>';
                $back .= "&nbsp;&nbsp;";
                ...
                class="tooltips"
                data-id="' . $data["id"] . '"
                onclick="Visa_types.delete(this);return false;"><i class="fa fa-times fa-2x text-danger"></i></a>';
        ...
        public function titles_check($titles = array(), $id = false){
            ...
            foreach ($titles as $key => $title) {
                ...
                $find = $this->visa_types->findTitle($where_array);
                if ($find) {

<?php
    ...
    class Haj_umrah_programs_slider_model extends CI_Model{

        private $_table = 'haj_umrah_programs_slider';

        public function __construct(){
            parent::__construct();
        }

        public function GetAll($program_id){
            $this->db->select('*');
            $this->db->from($this->_table);
            $this->db->where('haj_umrah_programs_id', $program_id);
            $query = $this->db->get();
            if ($query->num_rows() > 0) {
                return $query->result();
            } else {

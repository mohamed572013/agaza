...
class Maka_madina_hotels_slider_model extends CI_Model {
    ...
    public function GetMaka_madina_hotels_slider($array_where = array()) {
        if (isset($array_where) && \count($array_where) > 0) {
            $this->db->where($array_where);
        }
        $this->db->order_by('id', 'ASC');
    ...
    public function GetAllMaka_madina_hotels_slider($array_where = array()) {
        if (isset($array_where) && \count($array_where) > 0) {
            $this->db->where($array_where);
        }
        $query = $this->db->get($this->_table, 100);

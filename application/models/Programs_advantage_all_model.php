<?php
	...
	class Programs_advantage_all_model extends CI_Model{

		private $_table = 'programs_advantage_all';
		...
		public function GetallProgramExtra_service($Programs){
			$this->db->select('programs_advantage.title_ar , programs_advantage_all.*');
		...
		public function GetPrograms_advantage_all($array_where = array()){
			if (isset($array_where) && \count($array_where) > 0) {
				$this->db->where($array_where);
			}
			$this->db->order_by('id', 'ASC');
		...
		public function GetAllPrograms_advantage_all($array_where = array()){
			if (isset($array_where) && \count($array_where) > 0) {
				$this->db->where($array_where);
			}
			$query = $this->db->get($this->_table, 100);

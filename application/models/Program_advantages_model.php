<?php
    ...
    class Program_advantages_model extends CI_Model{

        private $table = 'programs_advantage_all';

        public function __construct(){
            parent::__construct();
        }

        public function find($programs_advantage_all_id){
            $this->db->select('*');
            $this->db->from($this->table);
            $this->db->where('id', $programs_advantage_all_id);
            $query = $this->db->get();
            if ($query->num_rows() > 0) {
                return $query->row();
            } else {

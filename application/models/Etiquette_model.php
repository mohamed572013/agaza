<?php
    ...
    class Etiquette_model extends CI_Model{

        private $_table = 'etiquette';
        ...
        public function getAllActiveEtiquette($branches_id, $limit = false, $offset = false){
            $this->db->select('*');
            $this->db->from($this->_table);   
            $this->db->where('is_active', 1);         
            $this->db->where('branches_id', $branches_id);
            //$this->db->where("order by the_order desc");

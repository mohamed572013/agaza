...
class Datatables
{
    ...
    {
        $ma = $this->explode(',', $match_replacement);
        if( count($ma) == 1 ) {
            $this->equal_columns[$column] = $ma[0];
        }
        $this->add_columns[$column] = array('content' => $content, 'replacement' => $ma);
        return $this;
    }
    /**
    ...
    private function get_ordering()
    {
        ...
        if ($this->ci->input->post('order'))
            foreach ($this->ci->input->post('order') as $key)
                if($this->check_cType()) {
                    if( isset( $this->equal_columns[$Data[$key['column']]['data']] ) )
                        $this->ci->db->order_by($this->equal_columns[$Data[$key['column']]['data']], $key['dir']);
                    else
    ...
    private function get_filtering()
    {
        ...
        if($sSearch != '')
            for($i = 0; $i < count($mColArray); $i++)
                if($mColArray[$i]['searchable'] == 'true' )
                    if($this->check_cType())
                        if( isset($this->select[$mColArray[$i]['data']]) ) $sWhere .= $this->select[$mColArray[$i]['data']] . " LIKE '%" . $sSearch . "%' OR ";
                        else
                            if( isset($this->columns[$i]) && isset($this->select[$this->columns[$i]]) ) $sWhere .= $this->select[$this->columns[$i]] . " LIKE '%" . $sSearch . "%' OR ";
        ...
        if($sSearch == '')
            for($i = 0; $i < count($mColArray); $i++)
                ...
                {
                    if($this->check_cType())
                        if( isset($this->select[$mColArray[$i]['data']]) )
                        {
                            $post_data = $this->ci->input->post($mColArray[$i]['data']);
                            if( $post_data != "" )
                            {
                                $sWhere .= $this->select[$mColArray[$i]['data']] . " LIKE '%" . $post_data . "%' OR ";
                            }
                        }
                        #else
                        #    if( isset($this->columns[$i]) && isset($this->select[$this->columns[$i]]) ) $sWhere .= $this->select[$this->columns[$i]] . " LIKE '%" . $sSearch . "%' OR ";
    ...
    {
        $replace_string = '';
        if( gettype($custom_val['content']) == "object" )
        {
            return $custom_val['content']( $row_data );
        }
        else if(isset($custom_val['replacement']) && is_array($custom_val['replacement']))
        {

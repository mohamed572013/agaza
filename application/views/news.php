								...
								?>
									...
									<div class="property-item clearfix hvr-float-shadow" data-aos="flip-left">
										<div class="list-property-img">
											<a href="<?= base_url("news/detail") . "/"  . $value->id . "-" . str_replace(" ", "_", $value->title_ar); ?>">
												<img src="<?= \base_url("uploads/news/$value->image") ?>" title="<?= $value->title_ar ?>" alt="<?= $value->title_ar ?>" />
											</a>
										</div>
										<div class="list-property-desc">
											<h4><a href="<?= base_url("news/detail") . "/"  . $value->id . "-" . str_replace(" ", "_", $value->title_ar); ?>"><?= $value->title_ar ?></a></h4>
											<p><?= \character_limiter(\strip_tags($value->body_ar), 150) ?></p>
											<div class="pro-btn-box">
												<a href="<?= base_url("news/detail") . "/"  . $value->id . "-" . str_replace(" ", "_", $value->title_ar); ?>" class="pro-ln-mor">التفاصيل</a>
											</div>
										</div>
									</div>
								</div> 

			...
			<div class="row">
				<div class="col-md-12">
					...
					<ol class="breadcrumb">
						...
						<li class="active"><a href="<?= \base_url('admin/extra_services/show') ?>"><?= $lang['extra_services']; ?></a></li>
					</ol>
					<!--Top breadcrumb start -->
				</div>
			</div>

			<a class="btn btn-sm btn-info pull-right" href="<?= \base_url("admin/extra_services/add") ?>"><?= $lang['add_new']; ?> </a>
			<br>
			<br>
			<div class="row">
				<div class="col-md-12">
					<div class="panel panel-default">
						<div class="panel-heading">
							<h3 class="panel-title"><?= $lang['extra_services']; ?></h3>
						</div>
											...
											if (count($page_list) > 0) {
												foreach ($page_list as $page_key => $page) {
													...
													<tr id="tr_<?= $page->id; ?>">
														<td><?= $i; ?></td>
														<td><?= $page->title_ar; ?></td>
														<td><?= $page->person_or_card == 0 ? "فرد" : "استماره"; ?></td>

														<td class="col-xs-2 col-sm-2 col-md-2 col-lg-2">
															<a class="btn btn-xs btn-warning" title="<?= $lang['edit']; ?>" href="<?= \base_url('admin/extra_services/edit') . '/' . $page->id; ?>"><i class="fa fa-pencil-square-o"></i> </a>
...
<script>
																	...
																	function delete_item_masa(title, id) {
																		...
																		$.ajax({
																			type: "post",
																			url: "<?= base_url("admin/extra_services/delete") ?>" + "/" + id,
																			success: function (data) {
																				if (data == "yes" || data == "	yes") {
																					...
																					$.amaran({
																						content: {
																							message: '<b> فشل فى  حذف الحدمة الاضافية</b>',
																							size: title,
																	...
																	function state_action_masa(title, id) {
																		...
																		$.ajax({
																			type: "post",
																			url: "<?= base_url("admin/extra_services/status") ?>" + "/" + id,
																			success: function (data) {
																				if (data == "yes" || data == "	yes") {
																					$.amaran({
																						content: {
																							message: '<b>تم  تعديل حالة </b>',
																							size: 'الحدمة الاضافية   #' + title,
																							file: '<b> </b>',
																							icon: 'glyphicon glyphicon-ok'
																						},
																						theme: 'default green',
																					...
																					$.amaran({
																						content: {
																							message: '<b> فشل فى  تغيير حالة  الحدمة الاضافية</b>',
																							size: title,
																							file: '<b> </b>',
																							icon: 'fa fa-times'
																						},

...
<section id="min-wrapper">
	<div id="main-content">
		<div class="container-fluid">
			...
			<a class="btn btn-sm btn-info pull-right" href="<?= \base_url("admin/hotel_rooms/add") ?>"><?= $lang['add_new']; ?> </a>
			<br>
			<br>
			<div class="row">
				<div class="col-md-12">
					<div class="panel panel-default">
						<div class="panel-heading">
							<h3 class="panel-title"><?= $lang['hotel_rooms']; ?></h3>
						</div>
											...
											if (count($page_list) > 0) {
												foreach ($page_list as $page_key => $page) {
													...
													<tr id="tr_<?= $page->id; ?>">
														<td><?= $i; ?></td>
 														<td><?= $page->title_ar; ?></td>
 														<td><?= $page->no_of_bed; ?></td>
 
														<td class="col-xs-2 col-sm-2 col-md-2 col-lg-2">
															<a class="btn btn-xs btn-warning" title="<?= $lang['edit']; ?>" href="<?= \base_url('admin/hotel_rooms/edit') . '/' . $page->id; ?>"><i class="fa fa-pencil-square-o"></i> </a>
...
function delete_item_masa(title, id) {
	...
	$.ajax({
		type: "post",
		url: "<?= base_url("admin/hotel_rooms/delete") ?>" + "/" + id,
...
function state_action_masa(title, id) {
	...
	$.ajax({
		type: "post",
		url: "<?= base_url("admin/hotel_rooms/status") ?>" + "/" + id,
		success: function (data) {
			if (data == "yes" || data == "	yes") {
				$.amaran({
					content: {
				...
				$.amaran({
					content: {
						message: '<b> فشل فى  تغيير حالة  الغرفة</b>',
						size: title,
						file: '<b> </b>',
						icon: 'fa fa-times'
					},

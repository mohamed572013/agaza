...
<section id="min-wrapper">
	<div id="main-content">
		<div class="container-fluid">
			...
			<a class="btn btn-sm btn-info pull-right" href="<?= \base_url("admin/flight_reservation/add") ?>"><?= $lang['add_new']; ?> </a>
			<br>
			<br>
			<div class="row">
				<div class="col-md-12">
										...
										<?php
											...
											if (count($page_list) > 0) {
												foreach ($page_list as $page_key => $page) {
													...
													<tr id="tr_<?= $page->id; ?>">
														<td><?= $i; ?></td>
 														<td><?= $page->travel_way; ?></td>
 														<td><?= $page->flight_company_name; ?></td>
 														<td><?= $page->going_date ."( ".$page->name_from_city ." ==> ".$page->name_to_city ." )"; ?></td>
 														<td><?= $page->return_date ."( ".$page->return_name_from_city ." ==> ".$page->return_name_to_city ." )"; ?></td>
 														<td><?= $page->passenger_num; ?></td>
 														<td><?= $page->price; ?></td>
 
														<td class="col-xs-2 col-sm-2 col-md-2 col-lg-2">
															<a class="btn btn-xs btn-warning" title="<?= $lang['edit']; ?>" href="<?= \base_url('admin/flight_reservation/edit') . '/' . $page->id; ?>"><i class="fa fa-pencil-square-o"></i> </a>


															<a class="btn btn-xxl "  title="<?= $lang['state']; ?>" onclick="state_action_masa('<?= $page->flight_company_name." " .$page->going_date  ; ?>',<?= $page->id; ?>)">
																<input     id="users_controll_show" title="<?= $lang['state']; ?>"  name="group_options" type="checkbox" class="js-switch "
																		   <?php if ($page->active == '1') echo 'checked'; ?> value="<?php echo $page->active; ?>"   onclick="state_action_masa('<?= $page->flight_company_name." " .$page->going_date     ; ?>',<?= $page->id; ?>)" >
															</a>
															<a class="btn btn-xs btn-danger" title="<?= $lang['delete']; ?>"   onclick="delete_action_masa('<?= $page->flight_company_name." " .$page->going_date     ; ?>',<?= $page->id; ?>)"  ><i class="fa fa-trash-o"></i>  </a>
...
function delete_item_masa(title, id) {
	...
	$.ajax({
		type: "post",
		url: "<?= base_url("admin/flight_reservation/delete") ?>" + "/" + id,
...
function state_action_masa(title, id) {
	...
	$.ajax({
		type: "post",
		url: "<?= base_url("admin/flight_reservation/status") ?>" + "/" + id,
		success: function (data) {
			if (data == "yes" || data == "	yes") {
				$.amaran({
					content: {
				...
				$.amaran({
					content: {
						message: '<b> فشل فى  تغيير حالة  رحلة </b>',
						size: title,
						file: '<b> </b>',
						icon: 'fa fa-times'
					},

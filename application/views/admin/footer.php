...
//                        }
            ...
            var handleChangeAddToRadio2 = function () {
                $('.addToTypeRadio').on('change', function () {
                    var type = $(this).val();
                    if (type == 'other') {
                        $('.branches-box').slideDown(500);
...
<?php
    ...
    if (!empty($_require)) {
        foreach ($_require as $key => $value) {
            ...
            if ($keys[0] == 'admin' && $keys[1] == 'js') {
                $path = 'assets/admin/rtl/js';
            }
            if ($keys[0] == 'admin' && $keys[1] == 'css') {
                $path = 'assets/admin/rtl/css';
            }
            if ($keys[0] == 'front' && $keys[1] == 'js') {
                $path = 'assets/front/js';
            }
            if ($keys[0] == 'front' && $keys[1] == 'css') {

			...
			<div class="row">
				<div class="col-md-12">
					...
					<ol class="breadcrumb">
						...
						<li class="active"><a href="<?= \base_url('admin/ads/show') ?>"> الاعلانات </a></li>
					</ol>
					<!--Top breadcrumb start -->
				</div>
			</div>

			<a class="btn btn-sm btn-info pull-right" href="<?= \base_url("admin/ads/add") ?>"><?= $lang['add_new']; ?> </a>
			<br>
			<br>
			<div class="row">
				<div class="col-md-12">
										...
										<?php
											...
											foreach ($page_list as $page_key => $page) {
												...
												<tr id="tr_<?= $page->id; ?>">
													...
													<td><?= $page->title_ar; ?></td>
													<td><?= \urldecode($page->url); ?></td>
													<td><img src="<?php echo \base_url("uploads/ads/$page->image")?>" class="img-responsive" style="width: 100px"/> </td>
													
													<td class="col-xs-2 col-sm-2 col-md-2 col-lg-2">
														<a class="btn btn-xs btn-warning" title="<?= $lang['edit']; ?>" href="<?= \base_url('admin/ads/edit') . '/' . $page->id; ?>"><i class="fa fa-pencil-square-o"></i> </a>
...
function delete_item_masa(title, id) {
	$.ajax({
		type: "post",
		url: "<?= base_url("admin/ads/delete") ?>" + "/" + id,
...
function state_action_masa(title, id) {
	...
	$.ajax({
		type: "post",
		url: "<?= base_url("admin/ads/status") ?>" + "/" + id,
		success: function (data) {
			if (data == "yes") {
				$.amaran({
					content: {
